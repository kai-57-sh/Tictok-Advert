...
Tictok Advert 是一个基于 Jetpack Compose 的单列广告信息流应用。应用默认从本地离线广告包加载 100 条中文广告，支持频道切换、下拉刷新、详情评论、互动统计和对话式搜索。
...
- [端侧模型说明](docs/AI广告推荐信息流-端侧模型demo.md)
...
│  │  ├─ data/                  # Room、Repository、离线包加载、Qwen 搜索
│  │  ├─ ui/                    # 首页、详情页、搜索页、统计页、ViewModel
│  │  └─ MainActivity.kt
│  ├─ src/main/assets/
│  │  └─ offline_ads/           # 广告 manifest、图片、视频资源
│  └─ src/test/java/com/example/ui/
│     └─ ...                    # 频道切换等单元测试
├─ docs/                        # 技术文档、测试材料、学习总结
├─ scripts/                     # 广告素材抓取与离线包生成脚本
├─ README.md
└─ spec.md
```
...
本项目的大部分编码工作由 AI 辅助完成，但所有代码与文档内容均经过人工复核、调试和确认后才最终提交。项目开发过程中，主要使用的开发与编码工具为 `Android Studio + Codex`，使用的模型为 `gpt-5.4`，整体采用 `Spec 驱动开发` 的方式推进：先明确需求、交互和约束，再进行方案拆解、代码生成、迭代修改、问题修复与结果验收。

AI 主要参与了页面实现、数据结构设计、交互逻辑补全、文档整理和部分重构工作；人工主要负责需求判断、方案取舍、代码审查、功能验证、编译运行检查以及最终结果把关。最终提交内容均以人工确认后的实际实现为准。
