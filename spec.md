...
- `Room` 负责保存 `ads`、`analytics_events` 与 `comments` 三张表。
...
- 若 `manifest.generatedAt` 与已导入版本不同，仓库会清空旧广告、旧评论和旧统计，再导入新的 bundle。
...
- 首页使用 `HorizontalPager + LazyColumn` 实现三频道信息流。
- 频道顺序固定为 `精选 -> 电商 -> 本地`，支持点击 Tab 和左右滑动切换。
- 每个频道独立保存 `LazyListState`，切回频道时恢复原滚动位置。
- 首页通过 `PullToRefreshBox` 下拉刷新当前频道，刷新后会重排当前频道内容并滚回顶部。
- 标签栏固定一行展示 `全部 + 最多 3 个热门标签 + 更多下拉`，溢出标签放入三列下拉菜单。
...
- 详情页评论保存在 Room `comments` 表。
- 评论作者固定为 `我`，内容为空时不可提交，最大长度为 `120` 字。
- 评论按时间倒序展示，重进详情页后仍可看到已提交内容。
     ...
     -> HomeScreen / DetailsScreen / SearchScreen / StatsScreen
        -> AdvertViewModel
           -> TictokAdvertRepository
              ...
              -> AppDatabase (Room: ads / analytics_events / comments)
              -> QwenSearchHelper
```
...
- 首页下拉刷新只重排当前频道的展示顺序，不触发离线包重新导入。
- 频道切换和广告列表展示以 Pager 当前页为准，避免 Tab 高亮和内容错位。
...
- 频道页序校验：`HomeScreenChannelMappingTest`
